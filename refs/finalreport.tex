...
\begin{table}[h]
    \centering
    \caption{Multilingual ASR metrics (HuBERT frozen, ASR-only)}
    \begin{tabular}{@{}lcc@{}}
        \toprule
...
\begin{table}[h]
    \centering
    \caption{Multilingual joint ASR+LID and PEFT (LoRA) vs.\ frozen ASR-only at 10 min. Test sets: \texttt{test\_10min\_lid} (joint), \texttt{test\_10min} (LoRA vs.\ frozen).}
    \begin{tabular}{@{}lccc@{}}
        \toprule
        \textbf{Track} & \textbf{CER (\%)} & \textbf{WER (\%)} & \textbf{Note} \\ \midrule
        ASR-only (frozen HuBERT) & 24.96 & 23.48 & baseline row \\
        ASR+LID joint & 26.33 & 25.49 & +LID token; slightly harder objective \\
        LoRA (HuBERT + adapters) & 24.95 & 23.66 & PEFT; \texttt{loralib} \\ \bottomrule
    \end{tabular}
\end{table}

Regarding \textbf{Language Identification (LID)}, the LID-only tracks (10 min and 1 h) completed; for those runs, standard word-level WER/CER tables are not interpretable (reference word count is zero), so LID quality is read from task-specific scores in the logs. The \textbf{ASR+LID (10 min)} joint model achieves \textbf{26.33\% CER} and \textbf{25.49\% WER} on \texttt{test\_10min\_lid}, confirming simultaneous transcription and language conditioning at the cost of a modest degradation versus ASR-only.

For \textbf{LoRA} (parameter-efficient fine-tuning on selected HuBERT attention projections), the 10-minute multilingual run yields \textbf{24.95\% CER} and \textbf{23.66\% WER} on \texttt{test\_10min}. Compared to frozen ASR-only at the same budget, CER is effectively \textbf{tied} (24.95 vs.\ 24.96), while WER is slightly higher (23.66 vs.\ 23.48). This suggests that, on this partial three-language setup, lightweight adapter training does not clearly outperform a frozen encoder plus CTC head; the \textbf{1 h LoRA} run remains to be reported. The \textbf{ASR+LID 1 h} joint run was not carried to a final scored checkpoint in time for this report.

\subsection*{Progress and Next Steps}
The pipeline advancement is estimated at \textbf{$\sim$80\%} for the ASR/multilingual track: monolingual benchmarks, multilingual frozen ASR, LID-only, ASR+LID (10 min), and LoRA (10 min) are scored; remaining items include:
\begin{itemize}
    \item Obtain final \textbf{LoRA 1 h} metrics and optionally rerun \textbf{ASR+LID 1 h} if a full one-hour joint table is required.
...
\begin{itemize}
    ...
    \item \textbf{Automation:} Orchestration scripts for sequential queuing (\texttt{run\_multi.sh}, multilingual PEFT queue/resume) and monolingual table export (\texttt{collect\_asr\_results.py} $\rightarrow$ \texttt{refs/ASR\_RESULTS\_TABLE\_eng1.md}); full multilingual tables are maintained in \texttt{refs/ASR\_RESULTS\_TABLE.md}.
\end{itemize}
