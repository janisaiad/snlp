...
\textbf{ASR+LID 10 min} completed in the resume queue, with final test metrics from
\texttt{exp/asr\_train\_asr\_s3prl\_10min\_multilingual\_10min\_lid/RESULTS.md}:
CER $=26.33\%$, WER $=25.49\%$ (test set \texttt{test\_10min\_lid}).

\textbf{ASR+LID 1 h} did not produce a final \texttt{RESULTS.md} (the run was stopped before completion).

\subsection{Multilingual LoRA (PEFT)}
LoRA adapters on HuBERT (ESPnet LoRA path) with \texttt{loralib} installed. On \texttt{test\_10min}, the multilingual LoRA 10~min model yields CER $=24.95\%$, WER $=23.66\%$
(\texttt{exp/asr\_train\_asr\_s3prl\_lora\_10min\_multilingual\_10min/RESULTS.md}).
Compared to frozen multilingual ASR 10~min (CER $24.96\%$, WER $23.48\%$), CER is \emph{essentially tied} and WER is slightly higher with LoRA.
LoRA 1~h results are pending until the corresponding \texttt{RESULTS.md} exists.
...
\begin{itemize}[leftmargin=*]
    ...
    \item Full metric tables: \texttt{refs/ASR\_RESULTS\_TABLE.md}; monolingual autogen: \texttt{refs/ASR\_RESULTS\_TABLE\_eng1.md}
...
\begin{enumerate}[leftmargin=*]
    \item Obtain \textbf{LoRA 1 h} \texttt{RESULTS.md}; optionally rerun \textbf{ASR+LID 1 h} if a full joint 1~h score is required.
    \item Refresh \texttt{refs/ASR\_RESULTS\_TABLE.md} and \texttt{refs/ASR\_RESULTS\_TABLE\_eng1.md} (the latter via \texttt{collect\_asr\_results.py} for monolingual rows only).
...
\section{Takeaway}
The monolingual benchmark is stable and documented. Multilingual HuBERT ASR (partial data) shows improved CER/WER versus the single-language \texttt{eng1} 10 min slice in the reported multilingual test setup, with the caveat of different language composition and data completeness. \textbf{ASR+LID 10 min} is completed (CER $26.33\%$, WER $25.49\%$ on \texttt{test\_10min\_lid}). \textbf{LoRA 10 min} is completed and matches frozen multilingual ASR on CER while WER is slightly worse. \textbf{ASR+LID 1 h} has no final table (run stopped). \textbf{LoRA 1 h} remains pending.
