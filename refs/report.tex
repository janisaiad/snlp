...
\textbf{ASR+LID 1 h} is running in the same queue
(\texttt{exp/asr\_train\_asr\_s3prl\_1h\_multilingual\_1h\_lid/}).
\textbf{LoRA multilingual ASR} (10 min then 1 h) is queued after ASR+LID 1 h.
...
\begin{enumerate}[leftmargin=*]
    \item Finalize \textbf{ASR+LID 1 h} and \textbf{LoRA} multilingual rows in tables once their \texttt{RESULTS.md} files are generated; rerun \texttt{collect\_asr\_results.py} after the queue finishes.
...
\section{Takeaway}
The monolingual benchmark is stable and documented. Multilingual HuBERT ASR (partial data) shows improved CER/WER versus the single-language \texttt{eng1} 10 min slice in the reported multilingual test setup, with the caveat of different language composition and data completeness. In the joint track, \textbf{ASR+LID 10 min is now completed} (CER $26.33\%$, WER $25.49\%$ on \texttt{test\_10min\_lid}); ASR+LID 1 h and LoRA runs remain the final pending steps in the active queue.
