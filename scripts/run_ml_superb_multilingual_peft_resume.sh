#!/usr/bin/env bash
# we resume the multilingual + PEFT queue after reboot: skip jobs that already have RESULTS.md
set -euo pipefail
...
# we mirror run_multi.sh suffix and asr_tag so exp paths match ESPnet asr_exp=exp/asr_${asr_tag}
asr_tag_for_job() {
  local cfg_rel="$1"
  local duration="$2"
  local lid="$3"
  local only_lid="$4"
  local suffix=""
  if [[ "${only_lid}" == "true" ]]; then
    suffix="_only_lid"
  elif [[ "${lid}" == "true" ]]; then
    suffix="_lid"
  fi
  local base
  base="$(basename "${cfg_rel}" .yaml)"
  printf "%s" "${base}_multilingual_${duration}${suffix}"
}

results_path_for_job() {
  local tag
  tag="$(asr_tag_for_job "$1" "$2" "$3" "$4")"
  printf "%s/exp/asr_%s/RESULTS.md" "${RECIPE}" "${tag}"
}
...
run_multi_job() {
  ...
  local name="$5"
  local res
  res="$(results_path_for_job "${cfg}" "${duration}" "${lid}" "${only_lid}")"
  if [[ -f "${res}" ]]; then
    log "SKIP ${name} ${duration} (already done: ${res})"
    return 0
  fi
...
log "=== ML-SUPERB multilingual + PEFT RESUME (NGPU=${ML_SUPERB_NGPU}) ==="
